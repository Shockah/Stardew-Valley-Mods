...
{
	internal static class SkillsPagePatches
	{
		...
		internal static void Apply(Harmony harmony)
		{
			harmony.TryPatch(
				...
				transpiler: new HarmonyMethod(typeof(SkillsPagePatches), nameof(SkillsPage_draw_Transpiler))
			);
			harmony.TryPatch(
				monitor: Monitor,
				original: () => AccessTools.Method(typeof(SkillsPage), nameof(SkillsPage.receiveLeftClick)),
				postfix: new HarmonyMethod(typeof(SkillsPagePatches), nameof(AnySkillsPage_receiveLeftClick_Postfix))
			);
			...
			if (Helper.ModRegistry.IsLoaded("spacechase0.SpaceCore"))
			{
				harmony.TryPatch(
					...
					transpiler: new HarmonyMethod(typeof(SkillsPagePatches), nameof(SpaceCore_NewSkillsPage_draw_Transpiler))
				);
				harmony.TryPatch(
					monitor: Monitor,
					...
					.Do(matcher =>
					{
						return matcher
							...
							.Insert(
								...
								new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(SkillsPagePatches), nameof(AnySkillsPage_draw_RecordSize)))
							);
					})
					.Do(matcher =>
					{
						...
						return matcher
							...
							.Insert(
								...
								new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(SkillsPagePatches), nameof(AnySkillsPage_draw_FinishDrawing)))
							);
					})
					.AllElements();
			}
		...
		private static IEnumerable<CodeInstruction> SpaceCore_NewSkillsPage_draw_Transpiler(IEnumerable<CodeInstruction> instructions)
		{
			...
			{
				Monitor.Log($"Could not patch SpaceCore methods - {ModManifest.Name} probably won't work.\nReason: {ex}", LogLevel.Error);
				return instructions;
			}
		}
