...
public sealed class ModEntry : BaseMod<ModConfig>
{
	...
	private Dictionary<long, IDictionary<string, IList<GiftEntry>>> GiftEntries = [];
	private readonly Dictionary<long, IList<Item>> ItemsToReturn = [];
	...
	public override void OnEntry(IModHelper helper)
	{
		...
		Characters = new(() =>
		{
			...
			var characters = Game1.characterData
				.Where(kvp => !GameStateQuery.IsImmutablyFalse(kvp.Value.CanSocialize))
				.Where(kvp => !antiSocialNpcs.ContainsKey(kvp.Key))
				.Select(kvp => (Name: kvp.Key, DisplayName: NPC.GetDisplayName(kvp.Key)))
				.ToArray();

			foreach (var (name, displayName) in characters)
				if (displayName is null)
					this.Monitor.Log($"Could not create configuration for character {name}, as its CharacterData is malformed.", LogLevel.Warn);
	...
	private void OnGameLaunched(object? sender, GameLaunchedEventArgs e)
	{
		...
		harmony.TryPatch(
			...
			Monitor,
			prefix: new HarmonyMethod(typeof(ModEntry), nameof(GiftShipmentController_GiftToNpc_Prefix)),
			postfix: new HarmonyMethod(typeof(ModEntry), nameof(GiftShipmentController_GiftToNpc_Postfix))
		);
		harmony.TryPatch(
			...
			Monitor,
			prefix: new HarmonyMethod(typeof(ModEntry), nameof(GiftShipmentController_CreateResponsePage_Prefix))
		);
		...
		harmony.TryPatch(
			...
			Monitor,
			postfix: new HarmonyMethod(typeof(ModEntry), nameof(NPC_getGiftTasteForThisItem_Postfix))
		);
		harmony.TryPatch(
			original: () => AccessTools.Method(typeof(NPC), nameof(NPC.receiveGift)),
			Monitor,
			postfix: new HarmonyMethod(typeof(ModEntry), nameof(NPC_receiveGift_Postfix))
		);
		harmony.TryPatch(
			original: () => AccessTools.Method(typeof(DialogueBox), nameof(DialogueBox.draw), [typeof(SpriteBatch)]),
			Monitor,
			transpiler: new HarmonyMethod(typeof(ModEntry), nameof(DialogueBox_draw_Transpiler))
		);
	}
	...
	private void RecordGiftEntryForNPC(Farmer player, string npcName, GiftEntry giftEntry)
	{
		...
		if (GameExt.GetMultiplayerMode() != MultiplayerMode.SinglePlayer)
		{
			...
			if (GameExt.GetMultiplayerMode() == MultiplayerMode.Server)
				...
				playersToSendTo = GameExt.GetHostPlayer() == player ? [] : [GameExt.GetHostPlayer()];
	...
	private static IEnumerable<CodeInstruction> NPC_tryToReceiveActiveObject_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator il)
	{
		try
		{
			return new SequenceBlockMatcher<CodeInstruction>(instructions)
				.Find(
					ILMatches.Ldarg(0).ExtractLabels(out var labels).CreateLabel(il, out var receiveGiftLabel),
					ILMatches.AnyLdloc,
					ILMatches.Ldfld("activeObj"),
					ILMatches.AnyLdloc,
					ILMatches.Ldfld("who"),
					ILMatches.LdcI4(1),
					ILMatches.LdcR4(1f),
				...
				.Insert(
					...
					new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(ModEntry), nameof(NPC_tryToReceiveActiveObject_Transpiler_ConfirmationDialogueCheck))),
		...
		try
		{
			return new SequenceBlockMatcher<CodeInstruction>(instructions)
				.Find(
					...
					ILMatches.AnyLdloc.CreateLdlocInstruction(out var ldlocResponseY),
					ILMatches.AnyLdcI4,
					...
					ILMatches.AnyLdloc.CreateLdlocInstruction(out var ldlocI),
					ILMatches.Instruction(OpCodes.Ldelem_Ref),
				...
				.Insert(
					...
					new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(ModEntry), nameof(DialogueBox_draw_Transpiler_DrawAccessory)))
				)
				.AllElements();
		}
		catch (Exception ex)
